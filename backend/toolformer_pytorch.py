...
def gumbel_noise(t):
    noise = torch.zeros_like(t, dtype=torch.float32).uniform_(0, 1)
...
def sample(
    ...
    *,
    ...
    print(f'Remaining iterations: {remain_iterations}')
    output = prime.clone()
    print(f"output: {output}")

    batch_indices = torch.arange(batch_size, device = device)
    batch_indices = rearrange(batch_indices, 'b -> b 1')
    position_index = torch.max(positions, dim = -1).values.item()
    ...
    bar = tqdm(range(remain_iterations), desc = 'Sampling', unit = 'token')
    for iteration in bar:
        logits = model.generate(output, pad_token_id=pad_id, eos_token_id=eos_token_id, do_sample=False, return_dict_in_generate=True, output_logits=True, max_new_tokens=1).logits
        logits = torch.stack(logits, dim=1)  # logits shape: (batch, seq_len, num_tokens)
        # print(f"Logits: {logits}")
        # # logits = model(output)
        # print(f"Iteration {iteration + 1}/{remain_iterations}, logits shape: {logits.shape}")
        ...
        # set the sampled tokens at the right cursor positions
        output = torch.concat([output, sampled], dim = -1)  # output shape: (batch, seq_len + sampled.shape[1])

        # increment positions
        position_index += sampled.shape[1]
        position_index = min(position_index, seq_len) # clamp to seq_len, since we do not want to go beyond the sequence length
...
def prompt_collate_fn(data, padding_value = 0):
    ...
    prompts = pad_sequence(prompts, padding_value = padding_value, padding_side='left')
...
class Toolformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        if not exists(api_start_id):
            api_start_id = self.tokenizer_encode(api_start_str)
            # assert len(api_start_id) == 1, f'the api start string `{api_start_str}` must be a single token, got {len(api_start_id)} tokens: {api_start_id}'
            api_start_id = api_start_id[-1]

        self.api_start_id = api_start_id

        if not exists(api_stop_id):
            api_stop_id = self.tokenizer_encode(api_stop_str)
            # assert len(api_stop_id) == 1, f'the api stop string `{api_stop_str}` must be a single token, got {len(api_stop_id)} tokens: {api_stop_id}'
            api_stop_id = api_stop_id[-1]
    ...
    def sample_model_with_api_calls(
        ...
        **kwargs
        ...
        if prime_is_str:
            prime_ids: list[int] = self.tokenizer_encode(prime)
            prime = torch.tensor(prime_ids).long()
        ...
        def call_apis(t: torch.Tensor):
            s: str = self.tokenizer_decode(t[0])
            s = invoke_tools_(s)
            t =  self.tokenizer_encode_to_tensor(s)
        ...
        output = sample_with_api_call(
            ...
            seq_len = self.model_seq_len,
            call_apis = call_apis,
            api_end_token_id = self.api_stop_id, # type: ignore
            occurrence = occurrence,
            **kwargs
        )
    ...
    def filter_by_api_responses(
        ...
        filtered_results = filter_tokens_with_api_response(
            ...
            api_start_token_id = self.api_start_id, # type: ignore
            api_end_token_id = self.api_stop_id # type: ignore
        )
    ...
    def finetune(
        ...
        dataset = FinetuneDataset(tokens = filtered_results) # type: ignore
    ...
    def forward(
        ...
        print(f'generated {len(data_with_api_calls)} sequences with API calls')
        print(f'example sequence with API calls: {data_with_api_calls[0]}')
        print(f'example sequence with API calls: {data_with_api_calls[1]}')
        print(f'example sequence with API calls: {data_with_api_calls[2]}')
        ...
        assert len(filtered_data_with_api_calls) > 0, 'your model failed to follow instructions and make API calls. please try a better model or do some better prompt engineering, generated output: \n' + '\n'.join(filtered_data)
